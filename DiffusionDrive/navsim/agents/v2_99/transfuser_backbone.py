...
from navsim.agents.v2_99.transfuser_config import TransfuserConfig

# Import VoVNet from module with hyphen in name
_vov_module = importlib.import_module('navsim.agents.v2_99.vov')
VoVNet = _vov_module.VoVNet


class FeatureInfo:
    """Wrapper to provide timm-compatible feature_info for VoVNet."""
    def __init__(self, channels, reductions):
        self.info = [
            {'num_chs': ch, 'reduction': red}
            for ch, red in zip(channels, reductions)
        ]


class RGBPermuteWrapper(nn.Module):
    """Wrapper to permute RGB channels before forwarding through a module."""
    def __init__(self, module):
        super().__init__()
        self.module = module

    def forward(self, x):
        # Permute RGB to BGR (DD3D weights expect BGR order)
        tmp = torch.zeros_like(x)
        tmp[:, 0] = x[:, 2]
        tmp[:, 1] = x[:, 1]
        tmp[:, 2] = x[:, 0]
        return self.module(tmp)


class VoVNetWrapper(nn.Module):
    """Wrapper for VoVNet to provide timm-compatible interface."""
    def __init__(self, checkpoint_path, prefix='img_backbone'):
        super().__init__()
        # Create VoVNet-99 with eSE
        vovnet = VoVNet(
            spec_name="V-99-eSE",
        ...
        # Extract all layers including stem (to match ResNet34's 5-layer output structure)
        from collections import OrderedDict
        self._layers = OrderedDict()

        # Wrap stem with RGB permutation (DD3D weights expect BGR)
        self._layers['stem'] = RGBPermuteWrapper(vovnet.stem)
        self._layers['stage2'] = vovnet.stage2
        self._layers['stage3'] = vovnet.stage3
        self._layers['stage4'] = vovnet.stage4
        self._layers['stage5'] = vovnet.stage5

        # Register as submodules
        for name, module in self._layers.items():
            self.add_module(name, module)

        # Create feature_info compatible with timm (5 layers to match ResNet34)
        # VoVNet stem: 128 channels at reduction 4
        # VoVNet99_eSE stages: [256, 512, 768, 1024] channels at reductions [4, 8, 16, 32]
        self.feature_info = FeatureInfo(
            channels=[128, 256, 512, 768, 1024],  # stem + 4 stages
            reductions=[4, 4, 8, 16, 32]  # stem at reduction 4, stages at 4/8/16/32
        )

        # Add return_layers attribute (5 layers: stem + 4 stages)
        # len(return_layers) = 5 > 4, will trigger stem separate forward in TransfuserBackbone
        self.return_layers = ["stem", "stage2", "stage3", "stage4", "stage5"]
    ...
    def forward(self, x):
        """Forward pass through all layers (used when called directly, not via iterator)."""
        for name, module in self._layers.items():
            x = module(x)
        return x
...
class TransfuserBackbone(nn.Module):
    ...
    def __init__(self, config: TransfuserConfig):
        ...
        # Check if v2_99 weights path is provided
        v2_99_weights_path = getattr(config, 'v2_99_weights_path', None)

        print(f"Loading v2_99 VoVNet model from: {v2_99_weights_path}")
        # Use VoVNet-99 with exact architecture from vov.py
        self.image_encoder = VoVNetWrapper(
            checkpoint_path=v2_99_weights_path,
            prefix='img_backbone.'
        )
