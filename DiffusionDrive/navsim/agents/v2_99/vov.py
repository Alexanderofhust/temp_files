...
class VoVNet(torch.nn.Module):
    ...
    def init_weights(self):
        ...
        prefix = self.init_cfg['prefix']
        pretrained_ckpt = torch.load(ckpt_path, map_location='cpu')['state_dict']
        keys_original = set(pretrained_ckpt.keys())
        pretrained_ckpt_used = dict()
        for k, v in pretrained_ckpt.items():
            if prefix in k:
                # backbone param
                k_new = k.replace(prefix, '')
                pretrained_ckpt_used[k_new] = v
        for k in keys_original:
            del pretrained_ckpt[k]
        msg = self.load_state_dict(
            pretrained_ckpt_used,
            strict=True
        )
        print('Loading V2-99 pretrained backbone', msg)
