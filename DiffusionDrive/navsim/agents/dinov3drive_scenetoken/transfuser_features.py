...
class TransfuserFeatureBuilder(AbstractFeatureBuilder):
    ...
    def _get_camera_feature(self, agent_input: AgentInput) -> torch.Tensor:
        """
        Extract the 4-camera input used by the DrivoR configuration.
        :param agent_input: input dataclass
        :return: stacked camera tensor with shape [num_cams, 3, H, W]
        """
        ...
        for cam in camera_list:
            if cam.image is None:
                raise ValueError("Missing camera image required for dinov3drive multi-camera input.")

            image = cv2.resize(cam.image, self._config.image_size)
