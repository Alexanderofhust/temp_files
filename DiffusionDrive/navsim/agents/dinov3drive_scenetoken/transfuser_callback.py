...
class TransfuserCallback(pl.Callback):
    ...
    def _visualize_model(
        ...
        if camera_feature.ndim == 5:
            camera = camera_feature[:, 0].permute(0, 2, 3, 1).numpy()
        else:
            camera = camera_feature.permute(0, 2, 3, 1).numpy()
        ...
        for sample_idx in range(self._num_rows * self._num_columns):
            ...
            front_camera = cv2.resize((camera[sample_idx] * 255).astype(np.uint8), (512, 128))
            plot[:128, :512] = front_camera
