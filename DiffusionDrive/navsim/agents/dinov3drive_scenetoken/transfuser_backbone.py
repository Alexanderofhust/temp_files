...
from navsim.agents.dinov3drive_scenetoken.transfuser_config import TransfuserConfig
from navsim.agents.dinov3drive_scenetoken.dinov3_image_encoder import DinoV3ImageEncoder
import timm
...
class TransfuserBackbone(nn.Module):
    ...
    def forward(self, image, lidar):
        ...
        image_scene_tokens = None

        for i in range(4):
            image_scene_tokens = all_image_features[i]
            lidar_features = self.forward_layer_block(lidar_layers, self.lidar_encoder.return_layers, lidar_features)
            image_scene_tokens, lidar_features = self.fuse_features(
                image_scene_tokens,
                lidar_features,
                i,
        ...
        if self.config.transformer_decoder_join:
            ...
            image_features = image_scene_tokens.view(batch_size, num_cameras, self.config.num_scene_tokens, -1).mean(dim=1)
    ...
    def fuse_features(self, image_scene_tokens, lidar_features, layer_idx, batch_size, num_cameras):
        """
        Perform a TransFuser feature fusion block using a Transformer module.
        :param image_scene_tokens: Scene tokens from the image branch with shape [B*num_cameras, num_scene_tokens, C]
        :param lidar_features: Features from the LiDAR branch
        :param layer_idx: Transformer layer index.
        :return: image scene tokens and lidar features with added features from the other branch.
        """
        image_embd_layer = image_scene_tokens.view(batch_size, num_cameras * self.config.num_scene_tokens, -1)
        lidar_embd_layer = self.avgpool_lidar(lidar_features)

        lidar_embd_layer = self.lidar_channel_to_img[layer_idx](lidar_embd_layer)

        image_scene_tokens, lidar_features_layer = self.transformers[layer_idx](image_embd_layer, lidar_embd_layer)
        ...
        return image_scene_tokens, lidar_features
...
class GPT(nn.Module):
    ...
    def __init__(self, n_embd, config, lidar_time_frames):
        ...
        self.image_token_length = self.config.num_cameras * self.config.num_scene_tokens
        self.pos_emb = nn.Parameter(
            torch.zeros(
                1,
                self.image_token_length + lidar_time_frames * self.config.lidar_vert_anchors * self.config.lidar_horz_anchors,
                self.n_embd,
            )
        )
    ...
    def forward(self, image_tensor, lidar_tensor):
        """
        Args:
            image_tensor (tensor): B, num_cameras * num_scene_tokens, C
        ...
        if image_tensor.dim() != 3:
            raise ValueError(f"Expected image scene tokens with shape [B, T, C], got {tuple(image_tensor.shape)}")
        ...
        image_tensor_out = x[:, : self.image_token_length, :]
        lidar_tensor_out = (
            x[
                :,
