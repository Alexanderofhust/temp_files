...
from .modules.conditional_unet1d import SinusoidalPosEmb
from .modules.multimodal_loss import LossComputer
from .score_module.scorer import Scorer
from .transfuser_backbone import TransfuserBackbone
...
class V2TransfuserModel(nn.Module):
    ...
    def _aggregate_pdm_score(self, pred_logit: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if self._config.score_noc_weight:
            pdm_score = pdm_score + self._config.score_noc_weight * F.logsigmoid(
                pred_logit["no_at_fault_collisions"]
            )
        if self._config.score_dac_weight:
            pdm_score = pdm_score + self._config.score_dac_weight * F.logsigmoid(
                pred_logit["drivable_area_compliance"]
            )
        if self._config.score_ddc_weight:
            pdm_score = pdm_score + self._config.score_ddc_weight * F.logsigmoid(
                pred_logit["driving_direction_compliance"]
            )
    ...
    def forward(self, features: Dict[str, torch.Tensor], targets: Dict[str, torch.Tensor] = None) -> Dict[str, torch.Tensor]:
        ...
        if self._config.use_score_head:
            ...
            score_feature = score_feature + trajectory_query
...
class TrajectoryHead(nn.Module):
    ...
    def forward_train(self, ego_query,agents_query,bev_feature,bev_spatial_shape,status_encoding, targets=None,global_img=None) -> Dict[str, torch.Tensor]:
        ...
        trajectory_reg_loss_dict = {}
        ret_traj_reg_loss = ego_query.new_tensor(0.0)
        for idx, (poses_reg, poses_cls) in enumerate(zip(poses_reg_list, poses_cls_list)):
            trajectory_reg_loss = self.loss_computer(poses_reg, poses_cls, targets, plan_anchor)
            trajectory_reg_loss_dict[f"trajectory_reg_loss_{idx}"] = trajectory_reg_loss
            ret_traj_reg_loss += trajectory_reg_loss
        ...
        return {
            ...
            "mode_scores_stage0": poses_cls_list[0],
            "mode_scores_stage1": poses_cls_list[-1],
            "trajectory_loss": ret_traj_reg_loss,
            "trajectory_reg_loss": ret_traj_reg_loss,
            "trajectory_reg_loss_dict": trajectory_reg_loss_dict,
        }
