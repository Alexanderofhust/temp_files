...
class TransfuserFeatureBuilder(AbstractFeatureBuilder):
    ...
    def _get_camera_feature(self, agent_input: AgentInput) -> torch.Tensor:
        """
        Extract 6 independent camera views from AgentInput
        :param agent_input: input dataclass
        :return: stacked camera images as torch tensor (6, 3, 448, 768)
        """

        cameras = agent_input.cameras[-1]

        # 定义6个相机
        camera_names = ['cam_f0', 'cam_l0', 'cam_r0', 'cam_l1', 'cam_r1', 'cam_b0']

        camera_tensors = []
        target_size = (768, 448)  # (width, height) for cv2.resize
        ...
        for cam_name in camera_names:
            ...
            camera_tensors.append(tensor_img)
