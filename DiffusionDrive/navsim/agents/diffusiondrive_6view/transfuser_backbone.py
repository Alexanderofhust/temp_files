...
from navsim.agents.diffusiondrive_6view.transfuser_config import TransfuserConfig
...
class TransfuserBackbone(nn.Module):
    ...
    def __init__(self, config: TransfuserConfig):
        ...
        # 多视角融合模块：SE通道注意力 + 多层卷积，保留空间维度
        in_ch = self.image_encoder.feature_info.info[start_index + 3]["num_chs"] * config.num_cameras
        out_ch = self.image_encoder.feature_info.info[start_index + 3]["num_chs"]
        self.camera_fusion = nn.Sequential(
            # SE通道注意力：学习6个视角中哪些特征更重要
            SEBlock(in_ch, reduction=4),
            # 第一步：1×1卷积压缩通道至中间维度
            nn.Conv2d(in_ch, out_ch * 2, kernel_size=1),
            nn.BatchNorm2d(out_ch * 2),
            nn.ReLU(inplace=True),
            # 第二步：3×3卷积引入空间感知（邻域信息聚合）
    ...
    def forward(self, image, lidar):
        ...
        Args:
            image: (B, 6, 3, 448, 768) - 6个相机图片,统一尺寸
            lidar: (B, C, H, W) - LiDAR BEV
        """
        # 处理多相机输入
        if len(image.shape) == 5:  # (B, 6, 3, 448, 768)
            batch_size, num_cams, C, H, W = image.shape
            # 将6个相机展平为 batch 维度: (B*6, 3, 448, 768)
        ...
        for i in range(4):
            ...
            # 单相机情况,保持原有的图像-LiDAR融合逻辑
            if num_cams == 1:
                image_features, lidar_features = self.fuse_features(image_features, lidar_features, i)

        # 最终层:融合6个相机特征，使用1×1卷积
        if num_cams > 1:
            # 将6个相机特征重组: (B*6, C, H, W) -> (B, 6*C, H, W)
            _, C_img, H_img, W_img = image_features.shape
            image_features_fused = image_features.view(
                batch_size, num_cams * C_img, H_img, W_img
            )
            # 1×1卷积融合: (B, 6*C, H, W) -> (B, C, H, W)，保留空间维度
            image_features = self.camera_fusion(image_features_fused)
        ...
        if self.config.transformer_decoder_join:
            ...
            image_features_pooled = self.global_pool_img(image_features)
            image_features_pooled = torch.flatten(image_features_pooled, 1)
            lidar_features_pooled = self.global_pool_lidar(lidar_features)
            lidar_features_pooled = torch.flatten(lidar_features_pooled, 1)

            if self.config.add_features:
                lidar_features_pooled = self.lidar_to_img_features_end(lidar_features_pooled)
...
class GPT(nn.Module):
    ...
    def __init__(self, n_embd, config, lidar_time_frames):
        ...
        # 图像: 768/32 = 24, 448/32 = 14, 所以特征图是 24x14 = 336
        # LiDAR: 256/32 = 8, 所以特征图是 8x8 = 64
        self.pos_emb = nn.Parameter(
            torch.zeros(
                1,
                self.seq_len * self.config.img_vert_anchors * self.config.img_horz_anchors  # 14 * 24 = 336
                + lidar_time_frames * self.config.lidar_vert_anchors * self.config.lidar_horz_anchors,  # 8 * 8 = 64
                self.n_embd,
            )
        )
