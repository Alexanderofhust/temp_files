...
def _maybe_disable_nuplan_ray_progress_bar() -> None:
    """Optionally silence the hard-coded `Ray objects` tqdm from nuplan."""
    if not _env_flag("NAVSIM_DISABLE_RAY_PROGRESS_BAR", False):
        return

    import nuplan.planning.utils.multithreading.ray_execution as ray_execution
    from tqdm import tqdm as base_tqdm

    if getattr(ray_execution, "_navsim_progress_bar_disabled", False):
        return
    ...
    ray_execution.tqdm = _silent_tqdm
    ray_execution._navsim_progress_bar_disabled = True
    logger.info("Disabled nuplan ray progress bars via NAVSIM_DISABLE_RAY_PROGRESS_BAR=1")
...
def initialize_ray(
    ...
    """
    _maybe_disable_nuplan_ray_progress_bar()
