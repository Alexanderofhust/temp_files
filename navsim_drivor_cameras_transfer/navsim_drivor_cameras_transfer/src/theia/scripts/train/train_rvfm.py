...
def should_compute_dist_loss(cfg: DictConfig) -> bool:
    """Whether the current main loss requires distloss."""
    return cfg.training.main_loss == "l1_dist"


def get_main_loss(losses: dict[str, torch.Tensor], cfg: DictConfig) -> torch.Tensor:
    """Select the main optimization loss from the computed loss dictionary."""
    if cfg.training.main_loss == "mse" or cfg.training.main_loss is None:
        return losses["mse_loss"]
    if cfg.training.main_loss == "cos_l1":
        return 0.9 * losses["cos_loss"] + 0.1 * losses["l1_loss"]
    if cfg.training.main_loss == "l1_dist":
        return 0.9 * losses["dist_loss"] + 0.1 * losses["l1_loss"]
    raise ValueError(f"Unsupported main_loss: {cfg.training.main_loss}")
...
def train(
    ...
    steps = 0
    compute_dist_loss = should_compute_dist_loss(cfg)
    ...
    for ep in range(epochs):
        ...
        train_loaders = get_frame_dataloader(
            ...
            persistent_workers=cfg.training.get('persistent_workers', False) if cfg.training.num_workers > 0 else False,
        ...
        metric_meters = create_meters(target_model_names, include_dist=compute_dist_loss)
        rvfm.train()
        ...
        for _ in train_tqdm:
            ...
            losses = rvfm.module.get_loss(pred, target_features_batch, compute_dist_loss=compute_dist_loss)
            main_loss = get_main_loss(losses, cfg)
...
def ddp_main(cfg: DictConfig) -> None:
    ...
    rvfm = RobotVisionFM(
        ...
        dist_loss_margin=cfg.training.get("dist_loss_margin", 0.25),
        dist_loss_use_margin=cfg.training.get("dist_loss_use_margin", True),
        **cfg.model.backbone,
    )
    ...
    train_dataset, train_dataset_expected_length = get_image_video_dataset(
        ...
        stats_prefix=cfg.dataset.stats_prefix,
        rank=rank,
    ...
    if rank == 0:
        ...
        wandb.init(project=cfg.logging.project, name=cfg.logging.run_identifier_prefix, config=OmegaConf.to_object(cfg), mode="online")
