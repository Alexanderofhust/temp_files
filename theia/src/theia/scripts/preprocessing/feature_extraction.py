...
def get_dataset(dataset_name: str, split: str, dataset_root: Optional[str] = None) -> tuple[Iterable, list[str]]:
    ...
    if dataset_name in ALL_OXE_DATASETS:
        ...
        input_shards = sorted(glob.glob(f"{dataset_dir}/*-{split}.tar"))
        print("input dataset shards", input_shards)
...
def get_images(ep: Any, subset: str) -> tuple[list[NDArray], Optional[list[str]]]:
    ...
    if isinstance(ep, list):
        # For webdataset (NavSim, ImageNet, etc.)
        # Images are already preprocessed to target size (1024x256 for NavSim)
        # No resize needed here
        if ep:
            print(f"DEBUG: Available keys in first sample: {list(ep[0].keys())}")

        return [step["image"] for step in ep], [step["__key__"] for step in ep]
    else:  # for oxe dataset, subset means multiple camera views
        images: list[NDArray] = []
        for step in ep["steps"]:
            # Changed from (224, 224) to (1024, 256) to support larger input resolution
            image = cv2.resize(step["observation"][subset].numpy(), (1024, 256))
            images.append(image)
        return images, None
    ...
    while shard_idx is not None:
        ...
        for subset in subsets:
            for k in shard_keys:
                ...
                print(f"output feature shard: {shard_path}")
