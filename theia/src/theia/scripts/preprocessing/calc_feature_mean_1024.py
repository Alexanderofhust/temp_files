...
"""
Calculate the channel-wise mean and var of extracted features on navsim_stitch dataset.
...
def main() -> None:
    ...
    for model in models:
        ...
        if model == "images" or model == "image" or model == "images_val":
            continue
        if os.path.exists(f"{args.output_path}/navsim_stitch_mean_{model}.npy"):
            continue
        ...
        np.save(f"{args.output_path}/navsim_stitch_mean_{model}.npy", model_mean_npy)
        ...
        np.save(f"{args.output_path}/navsim_stitch_var_{model}.npy", model_var.numpy())
