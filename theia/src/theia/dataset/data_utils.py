...
def load_feature_stats(
    dataset_root: str, feature_models: list[str], stats_prefix: str = "navsim_stitch"
    ...
    Args:
        ...
        stats_prefix (str): prefix of the stats files. Defaults to "navsim_stitch".
    ...
    for model in feature_models:
        ...
        feature_means[model] = torch.from_numpy(np.load(osp.join(dataset_root, f"{stats_prefix}_mean_{model_name}.npy"))).to(
            torch.bfloat16
        )
        feature_vars[model] = torch.from_numpy(np.load(osp.join(dataset_root, f"{stats_prefix}_var_{model_name}.npy"))).to(
            torch.bfloat16
        )
...
def get_image_video_dataset(
    ...
    stats_prefix: str = "navsim_stitch",
    seed: Optional[int | str] = 0,
    shuffle: bool = False,
    world_size: int = 1,
    **kwargs: Any,
    ...
    if feature_norm:
        feature_means, feature_vars = load_feature_stats(dataset_root, feature_models, stats_prefix)
