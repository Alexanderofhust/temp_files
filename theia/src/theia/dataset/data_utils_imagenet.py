...
def load_feature_stats(
    ...
    for model in feature_models:
        ...
        # feature_means[model] = torch.from_numpy(np.load(osp.join(dataset_root, f"imagenet_{model_name}.npy"))).to(
        #     torch.bfloat16
        # )
        # feature_vars[model] = torch.from_numpy(np.load(osp.join(dataset_root, f"imagenet_{model_name}.npy"))).to(
        #     torch.bfloat16
        # )
        feature_means[model] = torch.from_numpy(np.load(osp.join(dataset_root, f"navsim_mean_{model_name}.npy"))).to(
            torch.bfloat16
        )
        feature_vars[model] = torch.from_numpy(np.load(osp.join(dataset_root, f"navsim_var_{model_name}.npy"))).to(
            torch.bfloat16
        )
