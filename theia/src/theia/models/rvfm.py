...
class RobotVisionFM(nn.Module):
    ...
    Attributes:
        ...
        image_size (int | tuple[int, int]): size of the image. Can be int (square) or tuple (height, width).
    """
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        self.backbone: nn.Module = build_backbone(backbone, pretrained, image_size=self.image_size, **kwargs)
    ...
    def forward_feature(self, x: torch.Tensor, **kwargs: Any) -> torch.Tensor:
        ...
        Args:
            x (torch.Tensor): input image. By default it accepts images 
                ...
                For 1024x256 input: [B, 256, 1024, 3] or [B, 3, 256, 1024]
    ...
    def forward(self, x: torch.Tensor, target_model_names: Optional[list[str]] = None, **kwargs: Any) -> dict[str, torch.Tensor]:
        ...
        Returns:
            dict[str, torch.Tensor]: features that match to each foundation model.
                ...
                For 1024x256 input with patch16: [B, 1024, C] or [B, C, 16, 64]
        """
...
if  __name__ == "__main__":
    # simple test
    model = RobotVisionFM(
        backbone="facebook/deit-small-patch16-224",  # 或 "reg-facebook/deit-small-patch16-224"
        pretrained=True,
        image_size=(256, 1024),  # (height, width) = (256, 1024)
    )
    dummy_input = torch.randint(0, 255, (2, 3, 256, 1024), dtype=torch.uint8)
    output = model(dummy_input, target_model_names=["test_model"])
    for k in output:
