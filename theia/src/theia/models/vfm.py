...
class VFMEncoder(nn.Module):
    ...
    def __init__(self, model_name: str, feature_reduce_method: Optional[str] = None, **kwargs: Any):
        ...
        elif model_name.lower() == "qwen/qwen3-vl-8b-instruct":
            model, processor = get_qwen3_vl_model(model_name, device="cpu")
    ...
    def get_feature(self, x: torch.Tensor, **kwargs: Any) -> torch.Tensor:
        ...
        elif self.model_name.lower() == "qwen/qwen3-vl-8b-instruct":
            cls_token, visual_tokens, _ = get_qwen3_vl_feature(self.model, self.processor, x)
            visual_tokens = visual_tokens.flatten(start_dim=2).transpose(1, 2)
            feature = torch.cat([cls_token, visual_tokens], dim=1)
