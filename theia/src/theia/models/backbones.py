...
class ViTEmbeddingsNoCLS(ViTEmbeddings):
    ...
    def interpolate_pos_encoding(self, embeddings: torch.Tensor, height: int, width: int) -> torch.Tensor:
        ...
        resolution images. Supports non-square images.
        ...
        if num_patches == num_positions and height == width:
            return self.position_embeddings[:, 1:]
            
        patch_pos_embed = self.position_embeddings[:, 1:]
        dim = embeddings.shape[-1]
        
        # Original position embeddings are square
        orig_size = int(math.sqrt(num_positions))
...
class ViTEmbeddingsReg(ViTEmbeddings):
    ...
    def interpolate_pos_encoding(self, embeddings: torch.Tensor, height: int, width: int) -> torch.Tensor:
        ...
        if num_patches == num_positions and height == width:
            return torch.cat([self.position_embeddings, self.reg_pos_embed], dim=1)
...
class DeiT(nn.Module):
    ...
    def __init__(
        ...
        image_size: int | tuple[int, int] = (256,1024),
    ):
        super().__init__()
        # Support both int and tuple for image_size
        if isinstance(image_size, int):
            self.image_size = (image_size, image_size)
        else:
            self.image_size = tuple(image_size)  # (height, width)
        
        self.patch_size = 16
        self.final_spatial = (self.image_size[0] // self.patch_size, self.image_size[1] // self.patch_size)
    ...
    def get_feature_size(
        ...
        with torch.inference_mode():
            x = torch.zeros((1, *self.image_size, 3), dtype=torch.uint8)
    ...
    def forward(
        ...
        Args:
            x (torch.Tensor): model input. Expected shape: [B, H, W, C] or [B, C, H, W]
                             For NavSim: [B, 256, 1024, 3] or [B, 3, 256, 1024]
            ...
            do_resize (bool): if do resizing in processor. Defaults to False (keep original size).
            interpolate_pos_encoding (bool): if interpolate the positional embedding.
                                            Defaults to True (required for non-224x224 inputs).
        ...
        Returns:
            torch.Tensor: model output. Shape: [B, 1+H*W, C]
                         For 1024x256 input with patch16: [B, 1+64*16, 384] = [B, 1025, 384]
        """
...
class DeiTNoCLS(nn.Module):
    ...
    def forward(
        ...
        Returns:
            torch.Tensor: model output. Shape: [B, H*W, C]
        """
        input = self.processor(
            x, return_tensors="pt", do_resize=do_resize, do_rescale=do_rescale, do_normalize=do_normalize
        ).to(self.model.device)
...
class DeiTReg(nn.Module):
    ...
    def forward(
        ...
        Returns:
            torch.Tensor: model output. Shape: [B, 1+H*W+N, C]
                         For 1024x256 input with patch16: [B, 1+64*16+7, 384] = [B, 1032, 384]
        """
        input = self.processor(
            x, return_tensors="pt", do_resize=do_resize, do_rescale=do_rescale, do_normalize=do_normalize
        ).to(self.model.device)
...
def build_backbone(model_name: str, pretrained: bool = False, image_size: int | tuple[int, int] = 224, **kwargs: Any) -> nn.Module:
    ...
    Args:
        ...
        image_size (int | tuple[int, int]): size of the image. Can be int (square) or tuple (height, width). Defaults to 224.
