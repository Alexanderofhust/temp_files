...
# File modified for SAM3 support.
...
class Sam3MaskDecoderWithFeature(SamMaskDecoder):
    """SAM3 Mask decoder with upscaled feature exposed."""
...
@dataclass
class Sam3ImageSegmentationWithFeatureOutput(ModelOutput):
    """SAM3 segmentation output plus features."""
...
class Sam3ModelWithFeature(SamModel):
    """SAM3 model with feature exposed."""

    def __init__(self, config: SamConfig):
        super().__init__(config)
        self.mask_decoder = Sam3MaskDecoderWithFeature(config.mask_decoder_config)
        self.post_init()
    ...
    def forward(
        ...
        **kwargs: Optional[dict[str, Any]],
    ) -> tuple | Sam3ImageSegmentationWithFeatureOutput:
        """SAM3 forward pass with feature returned"""
        ...
        return Sam3ImageSegmentationWithFeatureOutput(
...
class Sam3ModelVisionFeature(SamModel):
    """SAM3 with only feature from the vision backbone."""
    ...
    def forward(
        ...
        **kwargs: Optional[dict[str, Any]],
        ...
        """SAM3 forward pass that only goes through vision backbone and returns visual feature."""
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        )
...
def get_sam3_feature(
    model: SamModel, processor: SamProcessor, images: list[np.ndarray], requires_grad: bool = False
) -> tuple[torch.Tensor, torch.Tensor]:
    """Get features from SAM3.

    Args:
        model (SamModel): SAM3 model.
        processor (SamProcessor): SAM3 input processor.
        images (list[np.ndarray]): images to be encoded, in RGB, uint8.
        requires_grad (bool): maintains gradient. Defaults to False.

    Returns:
        tuple[torch.Tensor, torch.Tensor]: (
            image_embeddings: feature from SAM3 visual encoder
            upscaled_image_embeddings: features from mask decoder
        )
    """
...
def get_sam3_model(
    model_name: str = "facebook/sam3", device: str | torch.device = "cuda", with_upscaled: bool = False
) -> tuple[Sam3ModelWithFeature, SamProcessor]:
    """Get SAM3 model and its input processor.

    Args:
        model_name (str, optional): name of SAM3 model. Defaults to "facebook/sam3".
    ...
    Returns:
        tuple[Sam3ModelWithFeature, SamProcessor]: SAM3 and its corresponding input processor
    """
    if with_upscaled:
        model = Sam3ModelWithFeature.from_pretrained(model_name).to(device)
    else:
        model = Sam3ModelVisionFeature.from_pretrained(model_name).to(device)
    processor = SamProcessor.from_pretrained(model_name)
    return model, processor


def print_feature_size(model_name: str = "facebook/sam3") -> None:
    """Print the size of features from SAM3.

    Args:
        model_name (str, optional): the name of SAM3 model. Defaults to "facebook/sam3".
    """
    import requests
    ...
    model, processor = get_sam3_model(model_name=model_name, device=device)
    image_embeddings, upscaled_embeddings = get_sam3_feature(model, processor, image_array)
