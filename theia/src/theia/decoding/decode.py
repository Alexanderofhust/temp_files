...
def load_feature_stats(
    ...
    for model in feature_models:
        ...
        feature_means[model] = torch.from_numpy(
            np.load(os.path.join(stat_file_root, f"navsim_mean_{model_name}.npy"))
        )
        feature_vars[model] = torch.from_numpy(np.load(os.path.join(stat_file_root, f"navsim_var_{model_name}.npy")))
