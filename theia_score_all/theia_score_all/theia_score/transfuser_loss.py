...
def transfuser_loss(
    ...
    diffusion_loss = predictions["diffusion_loss"] if "diffusion_loss" in predictions else 0

    final_score_loss = trajectory_score = best_score = None
    score_terms = {}
    if scoring_function is not None and "pred_logit" in predictions and "proposals" in predictions:
        ...
        trajectory_score = final_scores[
            torch.arange(len(final_scores), device=final_scores.device),
            top_proposals,
        ].mean()
        best_score = best_scores.mean()
    else:
        final_score_loss = trajectory_loss.new_tensor(0.0)
    ...
    if "trajectory_loss_dict" in predictions:
        loss_dict.update(predictions["trajectory_loss_dict"])
    return loss_dict
