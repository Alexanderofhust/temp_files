...
warnings.filterwarnings("ignore", message="invalid value encountered in arccos", category=RuntimeWarning)
...
def _load_metric_cache(metric_cache_path: str):
    with lzma.open(metric_cache_path, "rb") as file:
        return pickle.load(file)


def get_scores(args):
    return [get_sub_score(arg["token"], arg["poses"], arg["test"]) for arg in args]


def get_sub_score(metric_cache_path, poses, test):
    metric_cache = _load_metric_cache(str(metric_cache_path))
    if not hasattr(metric_cache, "pdm_progress"):
        raise AttributeError(
            "Score supervision requires train_metric_cache with pdm_progress. "
            "Generate it with run_train_metric_caching.py."
        )
