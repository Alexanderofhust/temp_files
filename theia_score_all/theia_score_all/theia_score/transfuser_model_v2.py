...
from navsim.agents.theia_score.modules.blocks import (
    GridSampleCrossBEVAttention,
    bias_init_with_prob,
    gen_sineembed_for_position,
    linear_relu_ln,
)
from navsim.agents.theia_score.modules.conditional_unet1d import SinusoidalPosEmb
from navsim.agents.theia_score.modules.multimodal_loss import LossComputer
from navsim.agents.theia_score.score_module.scorer import Scorer
from navsim.agents.theia_score.transfuser_backbone import TransfuserBackbone
from navsim.agents.theia_score.transfuser_config import TransfuserConfig
from navsim.agents.theia_score.transfuser_features import BoundingBox2DIndex
from navsim.common.enums import StateSE2Index


def _gather_top_trajectory(proposals: torch.Tensor, scores: torch.Tensor) -> torch.Tensor:
    top_idx = scores.argmax(dim=-1)
    top_idx = top_idx[..., None, None, None].repeat(1, 1, proposals.shape[2], proposals.shape[3])
    return torch.gather(proposals, 1, top_idx).squeeze(1)
...
class V2TransfuserModel(nn.Module):
    def __init__(self, config: TransfuserConfig):
        ...
        if config.use_score_head:
            self._score_projection = nn.Sequential(
                nn.Linear(config.trajectory_sampling.num_poses * 3, config.tf_d_ffn),
                nn.ReLU(),
                nn.Linear(config.tf_d_ffn, config.tf_d_model),
            )
            score_decoder_layer = nn.TransformerDecoderLayer(
                d_model=config.tf_d_model,
                nhead=config.score_num_heads,
                dim_feedforward=config.tf_d_ffn,
                dropout=config.tf_dropout,
                batch_first=True,
            )
            self._score_attention = nn.TransformerDecoder(score_decoder_layer, config.score_num_layers)
            self._scorer = Scorer(config)

        self.bev_proj = nn.Sequential(*linear_relu_ln(256, 1, 1, 320))

    def _aggregate_pdm_score(self, pred_logit: Dict[str, torch.Tensor]) -> torch.Tensor:
        pdm_score = torch.zeros_like(pred_logit["ego_progress"])
        if self._config.score_noc_weight:
            pdm_score = pdm_score + self._config.score_noc_weight * F.logsigmoid(pred_logit["no_at_fault_collisions"])
        if self._config.score_dac_weight:
            pdm_score = pdm_score + self._config.score_dac_weight * F.logsigmoid(pred_logit["drivable_area_compliance"])
        if self._config.score_ddc_weight:
            pdm_score = pdm_score + self._config.score_ddc_weight * F.logsigmoid(pred_logit["driving_direction_compliance"])

        weighted_score = (
            self._config.score_ttc_weight * pred_logit["time_to_collision_within_bound"].sigmoid()
            + self._config.score_ep_weight * pred_logit["ego_progress"].sigmoid()
            + self._config.score_comfort_weight * pred_logit["comfort"].sigmoid()
        )
        return pdm_score + weighted_score.clamp_min(1e-8).log()
    ...
    def forward(self, features: Dict[str, torch.Tensor], targets: Dict[str, torch.Tensor] = None) -> Dict[str, torch.Tensor]:
        ...
        output.update(self._agent_head(agents_query))

        if self._config.use_score_head:
            proposals = trajectory["proposals"]
            score_query = self._score_projection(proposals.detach().reshape(batch_size, proposals.shape[1], -1))
            score_feature = self._score_attention(score_query, keyval)
            pred_logit = self._scorer(score_feature)
            pdm_score = self._aggregate_pdm_score(pred_logit)

            output["pred_logit"] = pred_logit
            output["pdm_score"] = pdm_score
            if self._config.score_select_trajectory:
                output["trajectory"] = _gather_top_trajectory(proposals, pdm_score)
...
class DiffMotionPlanningRefinementModule(nn.Module):
    def __init__(self, embed_dims=256, ego_fut_ts=8, ego_fut_mode=20, if_zeroinit_reg=True):
        ...
        self.plan_cls_branch = nn.Sequential(*linear_relu_ln(embed_dims, 1, 2), nn.Linear(embed_dims, 1))
...
class TrajectoryHead(nn.Module):
    ...
    def forward_train(self, ego_query, agents_query, bev_feature, bev_spatial_shape, status_encoding, targets=None, global_img=None) -> Dict[str, torch.Tensor]:
        ...
        proposals = poses_reg_list[-1]
        mode_scores = poses_cls_list[-1]
        best_reg = _gather_top_trajectory(proposals, mode_scores)
        return {
            "trajectory": best_reg,
            "proposals": proposals,
    ...
    def forward_test(self, ego_query, agents_query, bev_feature, bev_spatial_shape, status_encoding, global_img) -> Dict[str, torch.Tensor]:
        ...
        best_reg = _gather_top_trajectory(poses_reg, poses_cls)
        return {
            "trajectory": best_reg,
            "proposals": poses_reg,
            "mode_scores": poses_cls,
        }
